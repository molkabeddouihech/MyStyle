...
continuerCommande(): void {
  ...
  this.verifierEmail().then((emailExiste) => {
    if (!emailExiste) {
      alert('Email non trouvé. Veuillez vous inscrire avant de passer commande.');
      return;
    }
  ...
  verifierEmail(): Promise<boolean> {
  return new Promise((resolve) => {
    fetch('http://localhost/projet_angular/back/verifier-email.php', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ email: this.emailClient })
    })
